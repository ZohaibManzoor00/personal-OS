# Personal OS
A personal operating system built to capture, organize, and grow everything I learn.

## Overview
This is my long-term knowledge hub for engineering, career growth, projects, and AI-powered workflows. Instead of scattered Google Docs and notes, everything lives in one connected system designed around how I learn and work.

## Features
- 🌳 **Hierarchical knowledge spaces** — a node-based tree of folders (spaces) and notes (pages) across four hubs: Learnings, Career, Projects & AI Workflows
- 📝 **Markdown notes with images** — GFM editor (CodeMirror + autosave/Vim mode) with drag-and-drop and cropped inline images
- 🔍 **Full-text search** — Postgres `tsvector` ranking with highlighted snippets, breadcrumbs, and auto scroll-to-match when you open a result
- 📑 **Automatic table of contents** — sticky "On this page" outline generated live from rendered headings, with active-section tracking

## Tech Stack
- Next.js
- React
- TypeScript
- tRPC
- Prisma
- PostgreSQL (Neon)
- Better Auth
- Cloudflare R2
- Tailwind CSS
- shadcn/ui

## Vision
The goal is to become a system that understands everything I've learned, built, and accomplished—unlocking intelligent workflows for learning, career growth, and productivity.
