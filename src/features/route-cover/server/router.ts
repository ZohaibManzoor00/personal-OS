...
const buildCoverKey = ({ userId, route, contentType }: { userId: string; route: string; contentType: string }) => {
...
export const routeCoverRouter = createTRPCRouter({
  ...
  attach: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!input.storageKey.startsWith(`route-covers/${userId}/${input.route}/`)) {
        throw new TRPCError({
          code: "BAD_REQUEST",
